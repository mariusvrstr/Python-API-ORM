...
class AdminService():
    ...
    def add_user(self, new_user_req: NewUserRequest):
        ...
        # self.context.refresh() - Investigating as alternative to early commit in shared session state
        account = self.account_repo.get_account(new_user_req.account_number)

        if (account == None):
            raise ValueError(f'Unable to add user to account [{new_user_req.account_number}] as it does not exist.')
